...
export default function DBCSCodec(codecOptions, iconv) {
  ...
  const encodeSkipVals = codecOptions.encodeSkipVals;
  if (encodeSkipVals) {
    const len = encodeSkipVals.length;
    for (let i = 0; i < len; ++i) {
      const val = encodeSkipVals[i];
