...
import { createError, isHttpError, HttpError }  from '../lib/deps/http-errors/index.js';
...
describe('isHttpError(val)', function () {
  ...
  describe('when val is a string', function () {
    it('should return false', function () {
      assert.strictEqual(isHttpError('foobar'), false)
    })
  })
...
describe('HTTP Errors', function () {
  ...
  it('should support err instanceof HttpError', function () {
    assert(createError(404) instanceof HttpError)
    assert((new createError['404']()) instanceof HttpError)
    assert((new createError['500']()) instanceof HttpError)
  })
