...
import { proxyaddr, alladdrs, compile }  from '../lib/deps/proxy-addr/index.js';
...
describe('proxyaddr.all(req, [trust])', function () {
  describe('arguments', function () {
    describe('req', function () {
      it('should be required', function () {
        assert.throws(alladdrs, /req.*required/)
      })
    })
    ...
    describe('trust', function () {
      it('should be optional', function () {
        var req = createReq('127.0.0.1')
        assert.doesNotThrow(alladdrs.bind(null, req))
      })
    })
  })
  ...
  describe('with no headers', function () {
    it('should return socket address', function () {
      var req = createReq('127.0.0.1')
      deepEqual(alladdrs(req), ['127.0.0.1'])
    })
  })
  ...
  describe('with x-forwarded-for header', function () {
    it('should include x-forwarded-for', function () {
      ...
      deepEqual(alladdrs(req), ['127.0.0.1', '10.0.0.1'])
    })
    ...
    it('should include x-forwarded-for in correct order', function () {
      ...
      deepEqual(alladdrs(req), ['127.0.0.1', '10.0.0.2', '10.0.0.1'])
    })
  })
  ...
  describe('with trust argument', function () {
    it('should stop at first untrusted', function () {
      ...
      deepEqual(alladdrs(req, '127.0.0.1'), ['127.0.0.1', '10.0.0.2'])
    })
...
describe('compile(trust)', function () {
  describe('arguments', function () {
    describe('trust', function () {
      it('should be required', function () {
        assert.throws(compile, /argument.*required/)
      })
      ...
      it('should accept a string', function () {
        assert.strictEqual(typeof compile('127.0.0.1'), 'function')
      })

      it('should reject a number', function () {
        assert.throws(compile.bind(null, 42), /unsupported trust argument/)
      })
      ...
      it('should accept IPv6', function () {
        assert.strictEqual(typeof compile('::1'), 'function')
      })

      it('should accept IPv4-style IPv6', function () {
        assert.strictEqual(typeof compile('::ffff:127.0.0.1'), 'function')
      })

      it('should accept pre-defined names', function () {
        assert.strictEqual(typeof compile('loopback'), 'function')
      })

      it('should accept pre-defined names in array', function () {
        assert.strictEqual(typeof compile(['loopback', '10.0.0.1']), 'function')
      })

      it('should reject non-IP', function () {
        assert.throws(compile.bind(null, 'blargh'), /invalid IP address/)
        assert.throws(compile.bind(null, '-1'), /invalid IP address/)
      })

      it('should reject bad CIDR', function () {
        assert.throws(compile.bind(null, '10.0.0.1/6000'), /invalid range on address/)
        assert.throws(compile.bind(null, '::1/6000'), /invalid range on address/)
        assert.throws(compile.bind(null, '::ffff:a00:2/136'), /invalid range on address/)
        assert.throws(compile.bind(null, '::ffff:a00:2/-46'), /invalid range on address/)
      })

      it('should not alter input array', function () {
        var arr = ['loopback', '10.0.0.1']
        assert.strictEqual(typeof compile(arr), 'function')
        deepEqual(arr, ['loopback', '10.0.0.1'])
      })
    })
  })
